...
class TestSpectroscopy(unittest.TestCase):
    ...
    def test_data(self):
        ...
        assert self.t_sample == 67497.603
        
        # correct shape
        assert self.sample_spectrum.shape[1] == self.n_channels
        
        # half lifes of all sources in seconds
        assert self.Eu152_source_specs['half_life'], 427075200.0
        assert self.Ba133_source_specs['half_life'], 331443360.0
        assert self.Na22_source_specs['half_life'], 82053518.4
    ...
    def test_energy_calibration(self):
        ...
        assert sorted(energy_calib_peaks.keys()) == sorted(self.energy_calib_peaks['channel'].keys())
        ...
        assert all(self.energy_calibration['perr'] / self.energy_calibration['popt'] <= 0.5)
    ...
    def test_efficiency_calibration(self):
        ...
        # check whether all peaks have been found
        assert sorted(efficiency_calib_peaks.keys()) == sorted(Eu152_expected.keys())
        ...
        # 50 % error on fit parameters
        assert all(self.efficiency_calibration['perr'] / self.efficiency_calibration['popt'] <= 0.5)
    ...
    def test_benchmark(self):
        ...
        assert all(ep in Na22_peaks for ep in Na22_expected.keys())

        # check whether all activities are in Bq
        assert all('normalized' == Na22_peaks[p]['activity']['type'] for p in Na22_peaks)
        
        # check whether all activities are calibrated for efficiency
        assert all(Na22_peaks[p]['activity']['calibrated'] for p in Na22_peaks)
        ...
        for na22_peak in Na22_peaks:
            if na22_peak in Na22_expected:
                ...
                assert low <= Na22_peaks[na22_peak]['peak_fit']['popt'][0] <= high
        ...
        assert 0.9 <= Na22_activity_meas['22_Na']['nominal'] / Na22_activity_theo[0] <= 1.0
        ...
        # check whether all expected peaks have been found from library
        assert all(ep in Ba133_peaks for ep in Ba133_expected.keys())
        
        # check whether all activities are in Bq
        assert all('normalized' == Ba133_peaks[p]['activity']['type'] for p in Ba133_peaks)
        
        # check whether all activities are calibrated for efficiency
        assert all(Ba133_peaks[p]['activity']['calibrated'] for p in Ba133_peaks)
        ...
        for ba133_peak in Ba133_peaks:
            if ba133_peak in Ba133_expected:
                ...
                assert low <= Ba133_peaks[ba133_peak]['peak_fit']['popt'][0] <= high, str(self.energy_calibration['accuracy'])
        ...
        # check to see at least 90% of the expected activity; only order of magnitude relevant
        assert 0.9 <= Ba133_activity_meas['133_Ba']['nominal'] / Ba133_activity_theo[0] <= 1.0
    ...
    def test_dose(self):
        ...
        assert np.isclose(dose['nominal'], 1.1330667863561383)
        assert np.isclose(dose['sigma'], 0.03766586624214202)
        assert dose['unit']=='uSv'

        dose = sp.get_dose(peaks_sample, distance=1)

        assert np.isclose(dose['nominal'], 1.8437455480798448)
        assert np.isclose(dose['sigma'], 0.05874337083851746)
        assert dose['unit'] == 'uSv/h'

        selected_peaks = select_peaks(['65_Zn', '48_V', '7_Be'], peaks_sample)

        assert len(selected_peaks) == 4

        selected_dose = sp.get_dose(selected_peaks, distance=50, time=2000)
        assert np.isclose(selected_dose['nominal'], 0.5343614796430098)
        ...
        assert np.isclose(selected_dose_time_1,selected_dose_time_2)
